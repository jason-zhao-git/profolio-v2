...
function getBreadcrumbs(pathname: string) {
  ...
  let breadcrumbs = [{ name: "Jingqiao Zhao", path: "/" }];
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const abbr = date
      ...
      .split(" ")
      .pop(); // Extracts the abbreviation like "PST"
  ...
  return (
    <div className="fixed top-0  w-full sm:w-full md:w-[80%] lg:w-[50%] bg-white dark:bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-md z-10 transition-colors duration-300">
      <header className="p-2 sm:p-4 sm:mx-0 mx-2 flex sm:flex justify-between items-center max-w-full ">
        ...
        <div className="flex items-center space-x-0 sm:space-x-4">
          <button
            ...
            className={`p-2 rounded-full transition-all duration-500 transform hover:rotate-[360deg]
    bg-transparent border-0 ring-0
    ${
      darkMode
        ? "hover:shadow-[0_0_20px_#f97316] text-orange-600"
        : "hover:shadow-[0_0_20px_#3b82f6] text-blue-600"
    }`}
            aria-label={
